...
2.  After a model is selected, we apply a procedure to fit or train the model.
...
Generally, regression is used when the response variable is quantitative. Classification is use when the response variable is qualitative. There is some overlap in methods, since some methods can perform regression or classification.
...
1.  For each of parts (a) through (d), indicate whether we would generally expect the performance of a flexible statistical learning method to be better or worse than an inflexible method. Justify your answer.
  ...
  + a. The sample size n is extremely large, and the number of predictors p is small.
    ...
    + A flexible model can take advantage of the large number of observations to make a detailed model that more closely fits the signal in the underlying data.
    
  + b. The number of predictors p is extremely large, and the number of observations n is small.
  
    + I'd apply dimension reduction to drive p well under n, and then apply an inflexible, parametric method like Ordinary Least Squares. This model would be less likely to overfit on a small number of observations.
...
2. Explain whether each scenario is a classification or regression problem, and indicate whether we are most interested in inference or prediction. Finally, provide n and p.
  ...
  + a. We collect a set of data on the top 500 firms in the US. For each firm we record profit, number of employees, industry and the CEO salary. We are interested in understanding which factors affect CEO salary.
    ...
    + The problem is regression, and we are interested in inferring the component factors. n is 500, and p is 3.
    
  + b. We are considering launching a new product and wish to know whether it will be a success or a failure. We collect data on 20 similar products that were previously launched. For each product we have recorded whether it was a success or failure, price charged for the product, marketing budget, competition price, and ten other variables.
  
    + The problem is binary classification. The n is 20, p is 13.
    
  + c. We are interested in predicting the % change in the USD/Euro exchange rate in relation to the weekly changes in the world stock markets. Hence we collect weekly data for all of 2012. For each week we record the % change in the USD/Euro, the % change in the US market, the % change in the British market, and the % change in the German market.
  
    + The % change problem is a regression problem. p is 3, n is 52.
...
3. We now revisit the bias-variance decomposition.
  ...
  + a. Provide a sketch of typical (squared) bias, variance, training error, test error, and Bayes (or irreducible) error curves, on a single plot, as we go from less flexible statistical learning methods towards more flexible approaches. The x-axis should represent the amount of flexibility in the method, and the y-axis should represent the values for each curve. There should be five curves. Make sure to label each one.
    ...
    + [Blue]{style="color: blue;"}: Bias

    + [Brown]{style="color: brown;"}: Variance

    + [Yellow]{style="color: yellow;"}: Training set MSE

    + [Green]{style="color: green;"}: Testing set MSE

    + Black: Irreducibile error (Bayes)

  + b. Explain why each of the five curves has the shape displayed in part (a).
  
    + [Yellow]{style="color: yellow;"}: As models become more flexible and complex, the training error is reduced, but with diminishing returns.

    + [Green]{style="color: green;"}: We often observe a U-shaped error in the holdout testing error. This is a combination of the Bias error above the least flexible model and the Variance error from over-fitting the most flexible models and the irreducible error component.

    + [Blue]{style="color: blue;"}: The bias error component, as observed in the test result

    + [Brown]{style="color: brown;"}: The variance error component, as observed in the test result
...
4. You will now think of some real-life applications for statistical learning.
  ...
  + a. Describe three real-life applications in which classification might be useful. Describe the response, as well as the predictors. Is the goal of each application inference or prediction? Explain your answer.
    ...
    + Infer the causes of customer service interaction success/failure. Predictors could be NLP summaries of phone call transcripts, metadata about the customer, and metadata about the customer's order.
    
    + Predict whether a customer order will end up being returned. The predictors could be information about the product, information about the customer, and information about the order process.

  + b. Describe three real-life applications in which regression might be useful. Describe the response, as well as the predictors. Is the goal of each application inference or prediction? Explain your answer.

    + Infer the causes of demand for a company's product. Independent predictors could be the time series of the target variable, economic indicators such as consumer spending, and marketing efforts.

    + Predict lead time for imported materials. Predictors could be the source of the materials (vendor, location), the shipping method, and weather indicators.
    
    + Predict the demand for a new product. Predictors could be information about similar product launches from the past, and data about the targeted customer type.
...
Notes on Boston housing data set:

The "black" variable is available in the original dataset from the MASS package. It appears that the authors of ISLR removed this column before importing it into their package.

From the ISLR2 package documentation:
...
Definition of "black" in the MASS dataset: "1000(Bk−0.63) where Bk is the proportion of blacks by town."

Criticisms of original dataset:
...
-   [https://medium.com/\@docintangible/racist-data-destruction-113e3eff54a8](https://medium.com/@docintangible/racist-data-destruction-113e3eff54a8){.uri}
...
`r knitr::include_url("https://www.youtube.com/embed/SY_t6zkh-0I")`
