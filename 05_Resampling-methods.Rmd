...
```{r fig5-1, echo=FALSE, cache=FALSE, fig.align='center', fig.cap= "Training- versus Test-Set Performance. Souce: https://hastie.su.domains/ISLR2/Slides/Ch5_Resampling_Methods.pdf}

knitr::include_graphics('./images/fig5_1.jpg')#, error = FALSE)
```
...
1.  To estimate the test error of a predictive model (i.e., to determine how well a given statistical learning procedure can be expected to perform on INDEPENDENT data)

2.  Sometimes we are interested in comparing different statistical learning methods or a single method using different levels of flexibility in order to id the method that results in the lowest test error. In this case, the actual value of the test MSE is not important - the location of the minimum point on the test MSE curve is.

-   Notice in figure above that although CV curves sometimes underestimate the true test MSE, all CV curves come close to identifying the correct level of flexibility (i.e., flexiblility level that corresponds to the smallest test MSE)
...
-   Simulated 100 pairs of data points (X, Y) four times and got four estimates for $\hat{\alpha}$ ranging from 0.532 to 0.657.
