...
- $\beta_{j}$ is the *average* effect on $Y$ from $X_{i}$ holding all other predictors fixed.  
...
6. *Collinearity*
   ...
   - Causes the (marginal) standard error to grow (and p-values to grow)
...
$$VIF(\hat{\beta_j}) = \frac{1}{1-R^2_{X_j|X_{-j}}}$$
    where $R^2_{X_j|X_{-j}}$ is the $R^2$ from a regression of $X_j$ onto all the otehr predictors.
