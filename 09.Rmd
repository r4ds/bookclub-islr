...
- Consider a matrix **X** of dimensions $n*p$, and a $y_{i} \in \{-1, 1\}$. We have a new observations, $x^*$, which is a vector $x^* = (x^*_{1}...x^*_{p})^T$ which we wish to classify to one of two groups.
- We will use a *separating hyperplane* to classify the observation.

## Visual Example of Using a Hyperplane to Classify

```{r hyperplane-class, echo=FALSE}
...
- The maximal margin classifier classifies $x^*$ based on the sign of $f(x^*) = \beta_{0} + \beta_{1}x^*_{1} + ... + \beta_{p}x^*{p}$.
...
- If $\epsilon_{i}$ = 0, the observation is on the correct side of the margin. If greater than 0, it is on the wronge side of margin. If greater than 1, it is on the wronge side of the hyperplane. 
...
$$f(x) =  \beta_{0} + \sum_{i \in S}K(x, x_{i})$$
...
- Assign test observation $x^*$ to the class $k$ for which $\beta_{0k} + ... + \beta_{pk}$ is largest.
